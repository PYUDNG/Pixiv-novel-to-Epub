...
!function() {
    ...
    let Popup = (function() {
        ...
        // ==========================================
        // 2. JSDoc 类型定义 (修正后完全适配 TypeScript)
        // ==========================================
        /**
         * 通用弹窗基础配置项
         * @typedef {Object} BaseOptions
         * @property {string} [header] 标题栏文本，缺省时使用内置默认文本
         * @property {boolean | 'auto'} [dark='auto'] 是否采用深色模式
         * @property {boolean} [backdropDismiss=true] 点击背景遮罩层是否自动关闭弹窗
         * @property {boolean} [seamless=false] 为 true 时不展示背景遮罩层并允许与页面其他部分交互
         * @property {HTMLElement | string} [parent=document.body] 宿主容器的父元素，支持元素引用或 CSS 选择器
         */

        /**
         * Prompt 专属配置项 (TS 兼容的交叉类型语法)
         * @typedef {BaseOptions & { value?: string }} PromptOptions
         */
        ...
        /**
         * 创建并运行一个标准样式的自定义弹窗
         * @param {BaseOptions} options 核心配置参数
         * @param {function(HTMLDivElement): void} contentBuilder 负责向内容区注入 DOM 的回调函数
         * @param {function(HTMLDivElement): void} footerBuilder 负责向底栏注入按钮的回调函数
         * @param {function(): void} [onClose] 弹窗非正常关闭（点击 X 或背景）时的回调
         * @private
         */
        function _createBasePopup(options, contentBuilder, footerBuilder, onClose) {
            const {
                header: customHeader,
                dark = 'auto',
            ...
            const textRes = CONST.Text.Popup;
            const headerText = typeof customHeader === 'string' ? customHeader : textRes.defaultTitle;

            let parentElement = document.body;
            if (parent instanceof HTMLElement) {
                parentElement = parent;
            } else if (typeof parent === 'string') {
                parentElement = document.querySelector(parent) || document.body;
            }

            const isSystemDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
            const useDarkMode = dark === 'auto' ? isSystemDark : dark;

            const colors = {
                bg: useDarkMode ? '#1e1e1e' : '#ffffff',
                text: useDarkMode ? '#d4d4d4' : '#333333',
                border: useDarkMode ? '#3c3c3c' : '#cccccc',
                headerBg: useDarkMode ? '#252526' : '#f3f3f3',
                buttonBg: useDarkMode ? '#333333' : '#e1e1e1',
                buttonHover: useDarkMode ? '#444444' : '#d0d0d0',
                primaryBg: useDarkMode ? '#007acc' : '#007acc',
                primaryHover: useDarkMode ? '#1f8ad2' : '#1f8ad2',
                primaryText: '#ffffff',
                inputBg: useDarkMode ? '#2d2d2d' : '#ffffff'
            };
            ...
            style.textContent = `
                .overlay {
                    ...
                    width: 100vw;
                    height: 100vh;
                    background: ${seamless ? 'transparent' : 'rgba(0, 0, 0, 0.5)'};
                    pointer-events: ${seamless ? 'none' : 'auto'};
                    z-index: 10000;
                    display: flex;
                    align-items: center;
            ...
            btnClose.title = textRes.closeTooltip;
            ...
            const dragStatus = { isDragging: false, startX: 0, startY: 0, initialX: 0, initialY: 0 };
            ...
            const onMouseDown = (e) => {
                ...
                dragStatus.initialX = rect.left;
                dragStatus.initialY = rect.top;
                
                win.style.margin = '0';
                win.style.left = `${dragStatus.initialX}px`;
                win.style.top = `${dragStatus.initialY}px`;
                overlay.style.alignItems = 'flex-start';
                overlay.style.justifyContent = 'flex-start';

                $AEL(unsafeWindow, 'mousemove', onMouseMove);
                $AEL(unsafeWindow, 'mouseup', onMouseUp);
            };

            const onMouseMove = (e) => {
                if (!dragStatus.isDragging) return;
                const dx = e.clientX - dragStatus.startX;
                const dy = e.clientY - dragStatus.startY;
                win.style.left = `${dragStatus.initialX + dx}px`;
                win.style.top = `${dragStatus.initialY + dy}px`;
            };
            ...
            $AEL(header, 'mousedown', onMouseDown);
            ...
            $AEL(overlay, 'click', (e) => {
                if (backdropDismiss && e.target === overlay) {
                    destroy();
                    if (onClose) onClose();
                }
            });
         ...
         * @param {BaseOptions} [options={}] 配置对象
        ...
        function alert(message = '', options = {}) {
            return new Promise((resolve) => {
                ...
                const contentBuilder = (contentEl) => {
                    const msgLabel = document.createElement('div');
                    msgLabel.className = 'hint';
                    msgLabel.textContent = message;
                    contentEl.appendChild(msgLabel);
                };

                const footerBuilder = (footerEl) => {
                    const btnConfirm = document.createElement('button');
                    btnConfirm.className = 'btn btn-primary';
                    btnConfirm.textContent = textRes.confirm;
                    
                    $AEL(btnConfirm, 'click', () => {
                ...
                const popupInstance = _createBasePopup(options, contentBuilder, footerBuilder, () => resolve());
            });
        }
         ...
         * @param {PromptOptions} [options={}] 配置对象
         * @returns {Promise<string | null>}
         */
        function prompt(hint = '', options = {}) {
            return new Promise((resolve) => {
                ...
                const contentBuilder = (contentEl) => {
                    ...
                    contentEl.appendChild(hintLabel);
                ...
                const footerBuilder = (footerEl) => {
                    ...
                    btnCancel.textContent = textRes.cancel;
                ...
                const popupInstance = _createBasePopup(options, contentBuilder, footerBuilder, () => resolve(null));
            });
        }
    ...
    (async function __MAIN__() {
        ...
        const CONST = {
            TextAllLang: {
                ...
                'zh-CN': {
                    ...
                    DownloadSuccess: '下载完成：{Name}',
                    NotNovelPage: '不支持在当前页面直接下载小说\n请进入小说阅读页面或系列小说目录页下载，或者使用合并下载功能手动输入小说ID/链接下载',
                    InputNovelIds: '每行输入一个小说的id或者链接，或者在同一行内用空格或逗号分隔多个小说的id或链接：',
                    InputFilename: '请设置文件名',
        ...
        function downloadEpub() {
            ...
            if (/^\/novel\/series\/\d+$/.test(pathname)) return downloadSeries();

            // Novel
            // https://www.pixiv.net/novel/show.php?id=18673574
            if (/^\/novel\/show\.php$/.test(pathname)) return downloadNovel();

            return Popup.alert(CONST.Text.NotNovelPage);
        }

        async function downloadCustom() {
            const darkmode = document.documentElement.getAttribute('data-theme') === 'dark';
            const input = (await Popup.prompt(CONST.Text.InputNovelIds, {
                header: CONST.Text.DownloadMultiple,
                seamless: true,
                dark: darkmode,
            }))?.trim();
            if (!input) return;
            const ids = input.split(/[ ,，\r\n]+/g).map(str =>
                parseInt(
                    str.startsWith('http') ?
                        new URLSearchParams(new URL(str).search).get('id') :
                        str,
                    10
                )
            );
            const data = await PixivAPI.novel(ids[0]);
            const filename = await Popup.prompt(CONST.Text.InputFilename, {
                header: CONST.Text.DownloadMultiple,
                value: data.body.title,
            });
            if (!filename) return Popup.alert(CONST.Text.DownloadCancelled, {
                header: CONST.Text.DownloadMultiple,
                dark: darkmode,
            });
            await downloadMultiple(ids, filename);

            Popup.alert(
                replaceText(CONST.Text.DownloadSuccess, {
                    '{Name}': filename,
                }), {
                    header: CONST.Text.DownloadMultiple,
                    dark: darkmode,
        ...
        async function downloadMultiple(novel_ids, filename) {
            ...
            await saveEpub(epub, filename, () => progress.finish());
        }
